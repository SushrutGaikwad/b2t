...
Note: `model` defaults to `gpt-4o` and is overridable. The `OpenAIConverter` is not unit-tested (no network); the `FakeConverter` covers the interface.
...
configurable value is the OpenAI `model` default (`gpt-4o`), called out
